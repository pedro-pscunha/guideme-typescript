...
- `TimeoutError` and `AbortError` are `DOMException`, not `TypeError`, so a timeout of any
  phase is never retried. Retrying one would multiply the wall time `timeout` promises.
...
| 5   | rubric-rules   | `test.each` of the refusals, 20 rows                      | SC3: the twelve rules, each with the contract sentence it pins; then seven of them again through a constructor (`choice`, `levels`, `chooseAmong`, `scoreLevels`, `noul().criteria()`), because "every rule holds on every path" is not proven by testing the renderer alone; then the two-fallback refusal. |
...
| 29  | tracing        | failure marks the span                                    | SC12: `error.type`, `otel.status_code`, `otel.status_description`.                                                                                                                                                                                                                                           |
